...
def test_signing_keys_on_upload(
    tmp_path,
    monitor_task,
    signing_gpg_metadata,
    rpm_package_signing_service,
    ...
    """Test that signing_keys is set on the package when uploaded with signing."""
    _, fingerprint, _ = signing_gpg_metadata
    ...
    assert [fingerprint] == package.signing_keys

    # Re-read repository to get the updated latest_version_href
    repository = rpm_repository_api.read(repository.pulp_href)

    # Verify signing_key filter returns the signed package
    results = rpm_package_api.list(
        repository_version=repository.latest_version_href,
        signing_key=fingerprint,
    ).results
    assert len(results) == 1
    assert results[0].pulp_href == package_href

    # Verify signing_key filter excludes packages signed with a different key
    fake_fingerprint = "0" * len(fingerprint)
    results = rpm_package_api.list(
        repository_version=repository.latest_version_href,
        signing_key=fake_fingerprint,
    ).results
    assert len(results) == 0


def test_signing_keys_on_modify(
    monitor_task,
    pulpcore_bindings,
    signing_gpg_metadata,
    rpm_package_signing_service,
    ...
    """Test that signing_keys is set on the package when added via modify."""
    monitor_task(pulpcore_bindings.OrphansCleanupApi.cleanup({"orphan_protection_time": 0}).task)
    ...
    assert [fingerprint] == signed_package.signing_keys
